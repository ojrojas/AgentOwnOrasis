...
import { IChatMessage, IChatRequest, IGenerateMessage, IGenerateRequest, IMessage } from '../types/chat-message.type';
...
export class OllamaAdapter implements IProviderApiService {
  ...
  async listModels(): Promise<{ models: string[] }> {
    const url = buildProviderUrl(this.config, 'models');
    return await this.http.get<{ models: string[] }>(url);
  }

  async generate(req: IGenerateRequest): Promise<IGenerateMessage> {
    const url = buildProviderUrl(this.config, 'generate');
